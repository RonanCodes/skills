...
2. **Link the code repo.** Claude Design reads a linked repository to learn existing components, architecture, and styling, which makes prototypes production-ready. Always pass the repo URL when one exists.
...
Hand the user the issue URL and a one-line "drop this URL into a new Claude Design high-fidelity project, paste the share link + screenshots back here when done."
